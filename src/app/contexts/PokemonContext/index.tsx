...
const PokemonProvider: React.SFC = ({ children }) => {
  ...
  const addPokemonsFromRequestResult = useCallback(
    (requestResultArray: RequestResultObject[]) => {
      ...
      requestResultArray.forEach((resultObject) => {
        const regexResult = new RegExp('/([0-9]+)/', 'g').exec(resultObject.url);
      ...
      Promise.all(pokemonPromises).then((pokemons) => {
        const parsedPokemons = pokemons.map((pokemonJson) => ({
          ...
          types: pokemonJson.types.map((typeInfo: { type: { name: string } }) => typeInfo.type.name),
  ...
  return <PokemonContext.Provider value={providerValue}>{children}</PokemonContext.Provider>;
};
